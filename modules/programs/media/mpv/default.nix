    ...
    (_: {
      programs.mpv = {
        ...
        config = {
          ...
          # UI - osd-*-color values come from DMS's matugen pipeline via
          # the include below. The rendered file is regenerated on every
          # wallpaper change and dark/light toggle.
          include = "~/.config/mpv/dank-colors.conf";
          osd-font-size = 32;
          osd-bar-h = 2;
          osd-bar-align-y = 0.5;
          osd-border-size = 2;
      ...
      # mpv OSD colors via DMS matugen. The include directive in
      # programs.mpv.config above pulls this file in at mpv startup.
      xdg.configFile."matugen/dms/templates/mpv.conf".text = ''
        osd-color="{{colors.on_surface.default.hex}}"
        osd-border-color="{{colors.surface.default.hex}}"
        osd-shadow-color="{{colors.shadow.default.hex}}"
        osd-back-color="{{colors.surface.default.hex}}"
      '';
      xdg.configFile."matugen/dms/configs/mpv.toml".text = ''
        [templates.mpv]
        input_path = '~/.config/matugen/dms/templates/mpv.conf'
        output_path = '~/.config/mpv/dank-colors.conf'
      '';
    })
  ];
}
